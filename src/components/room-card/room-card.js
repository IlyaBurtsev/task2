...
export function initRoomCards(room, serviceInfo) {
  ...
  const roomNumberToString =
    '<span class="room-card__number">№</span> ' +
    room.getRoomNumber() +
    (room.isLuxury ? ' <span class="room-card__additional">Люкс</span>' : '');

  roomNumber.innerHTML = roomNumberToString;
  roomNumber.nextElementSibling.innerHTML = `<span>${formatter.format(
    price
  )}₽</span> в сутки`;
  ...
  function setPriceInfo(selectedDates) {
    let days = getDays(selectedDates);

    roomInfo.innerHTML = `${formatter.format(price)}₽ x ${days} суток`;
    roomPrice.innerHTML = `${formatter.format(price * days)}₽`;

    if (discount != 0) {
      roomInfo.nextElementSibling.innerHTML = `Сбор за услуги: скидка ${formatter.format(
        discount
      )}₽`;
    }
    ...
    roomPrice.nextElementSibling.nextElementSibling.innerHTML = `${formatter.format(
      addServiceFee
    )}₽`;

    roomPrice.nextElementSibling.nextElementSibling.nextElementSibling.innerHTML = `${formatter.format(
      price * days + serviceFee + addServiceFee - discount
    )}₽`;
  }
