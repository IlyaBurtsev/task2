...
export class AirDatepickerCustom extends AirDatepicker {
	constructor(inputArrival, inputDeparture, selectedDates) {
		super(inputArrival, {
			range: true,
...
export function initDropdownDate(inputArrival, inputDeparture, selectedDates=[], startDate) {
	...
	let dp = new AirDatepicker(inputArrival, {
		...
		onSelect() {
			if (dp.selectedDates[1]) {
				// $inputDeparture.value = dp.formatDate(dp.selectedDates[1], dp.dateFormat);
				// $inputDeparture.value = dp.selectedDates[1];
			}
		}
	...
	if(selectedDates[1]){
		// $inputDeparture.value = selectedDates[1];
	}
